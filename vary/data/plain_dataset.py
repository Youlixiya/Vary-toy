...
class PlainDataset(BaseDataset):
    ...
    def __init__(self, datasets, tokenizer, multimodal_cfg):
        super(PlainDataset, self).__init__(datasets, tokenizer, multimodal_cfg)
        # v0 version format conversation
        conversation_lib.default_conversation = conversation_lib.conv_templates["default"]
        logging.warning("Formatting inputs into conversation type: Plain")
    ...
    def token_processor(self, sources):
        ...
        for target, source in zip(targets, sources):
            tokenized_len = len(self.tokenizer(source[0]['value']))
